  ...
  const instruction = useMemo(
    () =>
      [
        ...
        '',
        '   아이디는 밑줄(_)까지 인스타에 적힌 그대로 적는다. 아이디를 백틱(`)으로 감싸서',
        '   __seung_ah__ 같은 아이디가 굵게 표시로 바뀌어 밑줄이 사라지는 일이 없게 한다.',
        '   예) `__seung_ah__` | 2.1만 | 홈트 기록 | 소개글에 10월 공구 일정',
        '',
        ...
        `   한 검색어에서 더 나올 게 없으면 '${keywords[0] ?? '브랜드명'}공구' · '${
          keywords[0] ?? '브랜드명'
        }마켓' ·`,
        `   '${keywords[0] ?? '브랜드명'}할인' 처럼 변형 키워드로 넓혀서 목표 인원을 채운다.`,
        `   목표 인원(${count}명)을 못 채운 채로 끝내지 말고, 검색어를 바꿔가며 더 찾는다.`,
      ].join('\n'),
