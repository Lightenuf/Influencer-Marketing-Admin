...
function StageCards({
  ...
  const onDragMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (self && other) {
      ...
      const passed =
        direction === 1 ? selfCentre > otherCentre : selfCentre < otherCentre;
      if (passed) {
        const next = [...order];
        ...
        return (
          <div key={collab.id} ref={registerCard(collab.id)}>
            <div
              ...
              style={{
                transform:
                  dragId === collab.id ? `translateY(${offset}px)` : undefined,
            ...
            >
              <Card className="p-3">
                ...
                <div className="mt-2 space-y-0.5 text-[11px] text-slate-400">
                  <p
                    className={clsx(isStale && "font-semibold text-amber-600")}
                  >
                    {`${waiting}일째 ${stage}`}
                    {isStale && " ⚠️"}
                  </p>
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            단계별 진행 상황과 오래 멈춰 있는 건을 확인하세요. ({STALE_DAYS}일
            이상 대기 시 경고)
          </p>
        </div>
